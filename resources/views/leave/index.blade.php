...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ $isAdmin ? 'Leave Applications — All Users' : 'Leave Applications' }}
            </h2>
            @unless ($isAdmin)
    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <x-filter-bar :action="route('leave.index')" :users="$users" :departments="$departments"
                          date-label="Leave dates (overlapping)" />
            ...
            <div class="bg-white overflow-hidden shadow-md-1 sm:rounded-lg">
                @if ($applications->isEmpty())
                    <div class="p-8 text-center text-gray-500">
                        ...
                        @unless ($isAdmin)
                            <a href="{{ route('leave.create') }}" class="text-indigo-600 hover:underline">Create one</a>.
                        @endunless
                    </div>
                @else
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($applications as $app)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-3 font-medium text-gray-900">{{ $app->dateLabel() }}</td>
                                    @if ($isAdmin)
                                    ...
                                    <td class="px-6 py-3 text-right space-x-3">
                                        @unless ($isAdmin)
                                            <a href="{{ route('leave.edit', $app) }}" class="text-gray-600 hover:underline">Edit</a>
                                        @endunless
                                        <a href="{{ route('leave.show', $app) }}" class="text-indigo-600 hover:underline">Preview</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
