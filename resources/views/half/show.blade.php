    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Half Day Leave Application — {{ $half->notice_date->format('d-m-Y') }}
            </h2>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-md-1 sm:rounded-lg p-4 flex flex-wrap items-center gap-4 text-sm">
                <span><span class="text-gray-500">Reason:</span> <strong>{{ $half->reason }}</strong></span>
                <span><span class="text-gray-500">Type:</span> <strong>{{ $half->leave_type }}</strong></span>
                @if ($half->sent_at)
                    <span class="ml-auto inline-flex items-center rounded-full bg-green-100 px-3 py-1 text-green-800">
                        ✓ Sent {{ $half->sent_at->format('d-m-Y H:i') }}
                    </span>
                @else
            ...
            <div class="bg-white shadow-md-1 sm:rounded-lg p-4 text-sm space-y-1">
                <div><span class="text-gray-500 w-16 inline-block">Subject:</span> <strong>{{ $half->subjectLine() }}</strong></div>
                <div><span class="text-gray-500 w-16 inline-block">To:</span>
                    {{ !empty($half->to_emails) ? implode(', ', $half->to_emails) : (optional(auth()->user()->manager)->email ?? '— no recipient / manager —') }}
                </div>
                @if (!empty($half->cc_emails))
                    <div><span class="text-gray-500 w-16 inline-block">CC:</span> {{ implode(', ', $half->cc_emails) }}</div>
                @endif
            </div>
            ...
            <div class="bg-white shadow-md-1 sm:rounded-lg p-6">
                <div class="flex items-center justify-between mb-3">
                    <h3 class="font-semibold text-gray-800">Half Day Leave Application Template</h3>
            ...
            <div x-data="{
                    ...
                    variant: 'indigo',
                    formId: null,
                    sendMsg: @js('E-mail this half-day-leave application to the recipients?'),
                ...
                <div class="bg-white shadow-md-1 sm:rounded-lg p-4 flex flex-wrap items-center gap-3">
                    <a href="{{ route('half.edit', $half) }}"
                       class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 md-ripple rounded font-medium text-sm text-gray-700 uppercase tracking-wider shadow-md-1 hover:bg-gray-50 hover:shadow-md-2">
                        Edit
                    </a>

                    <form id="sendForm" method="POST" action="{{ route('half.send', $half) }}">
                        @csrf
                        <x-primary-button type="button"
                            @click="ask({ title: 'Send application', message: sendMsg, confirmLabel: 'Send', variant: 'indigo', formId: 'sendForm' })">
                            {{ $half->sent_at ? 'Re-send' : 'Send' }}
                        </x-primary-button>
                    </form>

                    <form id="deleteForm" method="POST" action="{{ route('half.destroy', $half) }}" class="ml-auto">
                        @csrf
                        @method('DELETE')
                        <button type="button"
                                @click="ask({ title: 'Delete application', message: 'Delete this half-day-leave application permanently? This action cannot be undone.', confirmLabel: 'Delete', variant: 'red', formId: 'deleteForm' })"
