    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Early Leave Application — {{ $early->notice_date->format('d-m-Y') }}
            </h2>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-md-1 sm:rounded-lg p-4 flex flex-wrap items-center gap-4 text-sm">
                <span><span class="text-gray-500">Reason:</span> <strong>{{ $early->reason }}</strong></span>
                <span><span class="text-gray-500">Leave time:</span> <strong>{{ $early->leave_time }}</strong></span>
                @if ($early->sent_at)
                    <span class="ml-auto inline-flex items-center rounded-full bg-green-100 px-3 py-1 text-green-800">
                        ✓ Sent {{ $early->sent_at->format('d-m-Y H:i') }}
                    </span>
                @else
            ...
            <div class="bg-white shadow-md-1 sm:rounded-lg p-4 text-sm space-y-1">
                <div><span class="text-gray-500 w-16 inline-block">Subject:</span> <strong>{{ $early->subjectLine() }}</strong></div>
                <div><span class="text-gray-500 w-16 inline-block">To:</span>
                    {{ !empty($early->to_emails) ? implode(', ', $early->to_emails) : (optional(auth()->user()->manager)->email ?? '— no recipient / manager —') }}
                </div>
                @if (!empty($early->cc_emails))
                    <div><span class="text-gray-500 w-16 inline-block">CC:</span> {{ implode(', ', $early->cc_emails) }}</div>
                @endif
            </div>
            ...
            <div class="bg-white shadow-md-1 sm:rounded-lg p-6">
                <div class="flex items-center justify-between mb-3">
                    <h3 class="font-semibold text-gray-800">Early Leave Application Template</h3>
            ...
            <div x-data="{
                    ...
                    variant: 'indigo',
                    formId: null,
                    sendMsg: @js('E-mail this early-leave application to the recipients?'),
                ...
                <div class="bg-white shadow-md-1 sm:rounded-lg p-4 flex flex-wrap items-center gap-3">
                    <form id="sendForm" method="POST" action="{{ route('early.send', $early) }}">
                        @csrf
                        <x-primary-button type="button"
                            @click="ask({ title: 'Send application', message: sendMsg, confirmLabel: 'Send', variant: 'indigo', formId: 'sendForm' })">
                            {{ $early->sent_at ? 'Re-send' : 'Send' }}
                        </x-primary-button>
                    </form>

                    <form id="deleteForm" method="POST" action="{{ route('early.destroy', $early) }}" class="ml-auto">
                        @csrf
                        @method('DELETE')
                        <button type="button"
                                @click="ask({ title: 'Delete application', message: 'Delete this early-leave application permanently? This action cannot be undone.', confirmLabel: 'Delete', variant: 'red', formId: 'deleteForm' })"
