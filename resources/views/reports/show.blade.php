    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Report Preview — {{ $report->report_date->format('d-m-Y') }}
            </h2>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-md-1 sm:rounded-lg p-4 flex flex-wrap items-center gap-4 text-sm">
                <span><span class="text-gray-500">Author:</span> <strong>{{ $report->user_name }}</strong></span>
                <span><span class="text-gray-500">Manager:</span> <strong>{{ $report->manager_name }}</strong>
                    @if ($report->manager)
                        <span class="text-gray-400">&lt;{{ $report->manager->email }}&gt;</span>
                    @endif
                </span>
                <span><span class="text-gray-500">Total:</span> <strong>{{ rtrim(rtrim(number_format($report->total_hours, 2), '0'), '.') }} hrs</strong></span>
                @if ($report->sent_at)
                    <span class="ml-auto inline-flex items-center rounded-full bg-green-100 px-3 py-1 text-green-800">
                        ✓ Sent {{ $report->sent_at->format('d-m-Y H:i') }}
                    </span>
                @else
            ...
            {{-- Cases tree (supplementary tracking data) --}}
            @if (! empty($report->cases))
                <div class="bg-white shadow-md-1 sm:rounded-lg p-4">
                    <h3 class="font-semibold text-gray-800 mb-3">Cases &amp; Task Breakdown</h3>
                    <div class="space-y-3">
                        @foreach ($report->cases as $case)
                            <div class="rounded-lg border border-gray-200 p-3">
                                <div class="flex flex-wrap gap-x-6 gap-y-1 text-sm">
                                    <span><span class="text-gray-500">System ID:</span> <strong>{{ $case['system_id'] ?: '—' }}</strong></span>
                                    <span><span class="text-gray-500">Case No.:</span> <strong>{{ $case['case_no'] ?: '—' }}</strong></span>
                                    @if (($case['time_h'] ?? null) !== null && $case['time_h'] !== '')
                                        <span><span class="text-gray-500">Time:</span> <strong>{{ $case['time_h'] }} h</strong></span>
                                    @endif
                                </div>

                                @if (! empty($case['task_components']))
                                    <ul class="mt-2 ms-4 ps-4 border-s-2 border-indigo-100 space-y-2">
                                        @foreach ($case['task_components'] as $task)
                                            <li>
                                                <div class="text-sm font-medium text-gray-800">{{ $task['name'] ?: '(untitled task)' }}</div>
                                                @if (! empty($task['sub_components']))
                                                    <ul class="mt-1 ms-4 ps-4 border-s-2 border-gray-100 space-y-1">
                                                        @foreach ($task['sub_components'] as $sub)
                                                            <li class="text-sm text-gray-600">
                                                                &bull; {{ $sub['name'] }}
                                                                @if (($sub['percent'] ?? null) !== null && $sub['percent'] !== '')
                                                                    <span class="text-indigo-600 font-medium">({{ $sub['percent'] }}%)</span>
                                                                @endif
                                                            </li>
                                                        @endforeach
                                                    </ul>
            ...
            <div class="bg-white shadow-md-1 sm:rounded-lg p-6">
                <div class="flex items-center justify-between mb-3">
                    <h3 class="font-semibold text-gray-800">Mail Template Preview</h3>
            ...
            @if ($report->user_id === auth()->id())
                <div x-data="{
                        ...
                        variant: 'indigo',
                        formId: null,
                        sendMsg: @js('Send this report by e-mail to '.$report->manager_name.'?'),
                    ...
                    <div class="bg-white shadow-md-1 sm:rounded-lg p-4 flex flex-wrap items-center gap-3">
                        <form id="sendForm" method="POST" action="{{ route('reports.send', $report) }}">
                            @csrf
                            <x-primary-button type="button"
                                @click="ask({ title: 'Send report', message: sendMsg, confirmLabel: 'Send', variant: 'indigo', formId: 'sendForm' })">
                                {{ $report->sent_at ? 'Re-send Mail to Manager' : 'Send Mail to Manager' }}
                            </x-primary-button>
                        </form>
                        ...
                        <form id="deleteForm" method="POST" action="{{ route('reports.destroy', $report) }}" class="ml-auto">
                            @csrf
                            @method('DELETE')
                            <button type="button"
                                    @click="ask({ title: 'Delete report', message: 'Delete this report permanently? This action cannot be undone.', confirmLabel: 'Delete', variant: 'red', formId: 'deleteForm' })"
